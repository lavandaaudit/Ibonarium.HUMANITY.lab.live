...
async function startEventFeed() {
    ...
    updateStatus('Запуск моніторингу подій (GDELT)...');

    // Real GDELT Fetch
    async function fetchGDELT() {
        try {
            updateStatus('Звернення до GDELT Real-time Stream...');
            ...
            if (data && data.articles && data.articles.length > 0) {
                updateStatus('GDELT: Потік даних активний.');
                data.articles.forEach(art => {
                    ...
                    // GDELT impact on global stats
                    globalStats.avgTension = Math.max(0.1, Math.min(0.9, globalStats.avgTension + (Math.random() * 0.02 - 0.01)));
                    globalStats.avgEconomy = Math.max(0.1, Math.min(0.9, globalStats.avgEconomy + (Math.random() * 0.01 - 0.005)));
                });
                updateGlobalDashboard();
                ...
                updateStatus('GDELT: Потік порожній, очікування...');
                generateSimulatedEvent();
            }
        } catch (e) {
            console.warn('GDELT Connection failed:', e);
            updateStatus('GDELT: Тимчасовий збій. Симуляція...');
            generateSimulatedEvent();
        }
    }
    ...
    setInterval(() => {
        // Higher probability for real GDELT news (50%)
        if (Math.random() > 0.5) fetchGDELT();
